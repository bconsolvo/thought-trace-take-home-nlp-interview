...
class ocrpytesseract:
    ...
    def jpg2text(self,drc):
        ...
        for i in im_list:
            ...
            text = pytesseract.image_to_string(imjpg, config=config) # Uses trained model from Tesseract to convert JPG image to a string of text
            scores = pytesseract.image_to_data(imjpg, output_type=pytesseract.Output.DATAFRAME, config=config) # scores for each word transcribed
